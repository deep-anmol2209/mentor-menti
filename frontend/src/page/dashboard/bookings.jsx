...

    {
      ...
      render: (text, record) =>
        record.status === "cancelled" ||
          ...
          <a href={text} target="_blank" rel="noopener noreferrer">Join now</a>
        ),
    },
  ...
  if (user.role === 'mentor' || user.role === 'student') {
    columns.push({
      ...
      render: (_, record) => {
        ...
        const isStudent = user.role === "student" && record.status === "reschedulerequest";
        // ✅ Skip rendering for cancelled or completed bookings
          ...
          const dropdownMenu = (
            <Menu>
              {isMentor && record.serviceType === "one-on-one" && (
                <Menu.Item key="reschedule" onClick={() => handleRescheduleModal(record)}>
                  Reschedule
                </Menu.Item>
              )}

              {isMentor && (
                <Menu.Item key="completed" onClick={() => handleMarkAsCompleted(record)}>
                  Completed
                </Menu.Item>
              )}

              <Menu.Item key="cancel" danger onClick={() => handleCancelBooking(record)}>
                Cancel
              </Menu.Item>
            </Menu>
          );
